...
GeneralSettings* GeneralSettings::instance = 0;
QMutex GeneralSettings::mutex;
...
void GeneralSettings::fillLanguageBox() const
{
    ...
    const QString creatorTrPath     = Utils::GetDataPath();
...
void GeneralSettings::setLanguage(const QString &locale)
{
    if (m_language != locale) {
        if (!locale.isEmpty()) {
	  QMessageBox::information((QWidget *) &MainWindow::getInstance(), tr("Restart required"),
