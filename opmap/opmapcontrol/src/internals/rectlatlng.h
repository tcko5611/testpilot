...
namespace internals {
struct INTERNALS_EXPORT RectLatLng {
public:
    static RectLatLng Empty;
    friend uint INTERNALS_EXPORT qHash(RectLatLng const & rect);
    friend bool INTERNALS_EXPORT operator==(RectLatLng const & left, RectLatLng const & right);
    friend bool INTERNALS_EXPORT operator!=(RectLatLng const & left, RectLatLng const & right);
