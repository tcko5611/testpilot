...
const double PureProjection::MAXPLONG = 2147483647;
...
double PureProjection::AdjustLongitude(double x)
{
    ...
    while (true) {
        if (qAbs(x) <= PI) {
            ...
            x = x - (Sign(x) * TWO_PI);
        } else if (((qlonglong)qAbs(x / TWO_PI)) < MAXPLONG) {
            x = x - (((qlonglong)(x / TWO_PI)) * TWO_PI);
        } else if (((qlonglong)qAbs(x / (MAXPLONG * TWO_PI))) < MAXPLONG) {
            x = x - (((qlonglong)(x / (MAXPLONG * TWO_PI))) * (TWO_PI * MAXPLONG));
        } else if (((qlonglong)qAbs(x / (DBLLONG * TWO_PI))) < MAXPLONG) {
